...
pubDatetime: 2020-11-25T14:19:30.000Z
tags: ["devops", "game-dev", "tutorial"]
description: "以往经历 我之前搭过两台Minecraft的服务器，一台是阿里云的1核2G内存5M带宽(9.5元/月)，一台是百度云的2核4G3M(30+元/月)，都是学生机比较便宜，百度云那台的基础价格是18元/月只有1M带宽，我是升了配置，记不清一共多少钱了。 阿里云那台用作创造的实验服务器还是可以的，百度云那..."
---
...
![](/images/mc-server-deploy/server_mc-1.png)

选了上海地区的服务器，还是用Ubuntu做系统

![](/images/mc-server-deploy/server_mc-2.png)

购买服务器后第一件事应该是重置密码，这里默认的用户名为`ubuntu`

![](/images/mc-server-deploy/server_mc-3.png)
...
![](/images/mc-server-deploy/server_mc-4.png)
...
3. 执行`python3 MCDRefored.py`试运行服务端
   ...
   ![](/images/mc-server-deploy/server_mc-5.png)

   可以看到运行成功，服务器内情况正常：

   ![](/images/mc-server-deploy/server_mc-6.png)

   服务器资源占用也很低

   ![](/images/mc-server-deploy/server_mc-7.png)

   空载时服务器负荷并不大

   ![](/images/mc-server-deploy/server_mc-8.png)
